...
SELECT
  ROW_NUMBER() OVER (ORDER BY kills_per_game DESC) AS kpgn, 
  ROW_NUMBER() OVER (ORDER BY deaths_per_game ASC) AS dpgn,
  ROW_NUMBER() OVER (ORDER BY assists_per_game DESC) AS apgn, 
  ROW_NUMBER() OVER (ORDER BY kda DESC) AS kdan, 
  ROW_NUMBER() OVER (ORDER BY gpm DESC) AS gpmn, 
  ROW_NUMBER() OVER (ORDER BY dpm DESC) AS dpmn, 
  ROW_NUMBER() OVER (ORDER BY cd10 DESC) AS cd10n, 
  ROW_NUMBER() OVER (ORDER BY game_length DESC) AS gln, 
  COUNT(*) OVER () as total,
  * FROM ( SELECT
...
GROUP BY p.player_name HAVING games >= 4 
 );
